...
class ApplicationsLogicController {
  ...
  func toggleAppearance(_ newAppearance: Application.Appearance,
    ...
    queue.async { [weak self] in
      ...
      DispatchQueue.main.asyncAfter(deadline: .now() + 1.0) {
        ...
        NSLog("noctOs: terminal output: (\(output))")
        self?.load()
      }
      ...
      NSLog("noctOs: New settings for \(application.name) = \(newAppearance)")
      NSLog("noctOs: Command: \(command)")
    }
  }
  ...
  private func applicationLocations() throws -> [URL] {
    ...
    // macOS default application directories.
    directories.append(applicationDirectory)
    directories.append(applicationDirectoryU)
    directories.append(homeDirectory.appendingPathComponent("Developer/Applications"))
    directories.append(applicationDirectoryD)
    directories.append(applicationDirectoryN)
    directories.append(applicationDirectoryND)    
    
    // Non-default application directories
    directories.append(coreServicesDirectory) // *Hopefully*, noctOS will exclude any non-application bundles, including Finder or some other miscellaneous system applications.
    ...
    directories.append(homeDirectory.appendingPathComponent("Library/Developer/Xcode/DerivedData"))
  ...
  private func parseApplicationUrls(_ appUrls: [URL],
    ...
    for (offset, url) in appUrls.enumerated() {
      ...
      // Checks if noctOs has enough permissions to change the appearance.
      let restricted = sip &&
